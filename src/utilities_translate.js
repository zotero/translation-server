...
*/
Zotero.Utilities.Translate.prototype.doGet = async function(urls, processor, done, responseCharset, requestHeaders) {
	var translate = this._translate;
	...
	if (responseCharset) {
		requestHeaders['accept-charset'] = responseCharset;
	}
	
	translate.incrementAsyncProcesses("Zotero.Utilities.Translate#doGet");
	var response = await Zotero.HTTP.request("GET", url, {headers: requestHeaders});
	try {
		if (processor) {
			processor(response.responseText, response, url);
		}
		
		if (urls > 0) {
			this.doGet(urls, processor, done, responseCharset, requestHeaders);
		} else {
			if (done) {
				done();
			}
 ...
 * @ignore
 */
Zotero.Utilities.Translate.prototype.doPost = async function(url, body, onDone, headers, responseCharset) {
	var translate = this._translate;
	url = translate.resolveURL(url);
	headers = headers || {};
	if (responseCharset) {
		headers['accept-charset'] = responseCharset;
	}
	translate.incrementAsyncProcesses("Zotero.Utilities.Translate#doPost");

	var response = await Zotero.HTTP.request("POST", url, {headers});
	try {
		onDone(response.responseText, response, url);
		translate.decrementAsyncProcesses("Zotero.Utilities.Translate#doGet");
	} catch(e) {
		translate.complete(false, e);
	}
